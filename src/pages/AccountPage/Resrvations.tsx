...
import { useGetAllBookingPlacesVendor } from "../../hook/useGetAllBookingPlacesVendor";
import { useGetAllBookingServiceVendor } from "../../hook/useGetAllBookingServiceVendor";
...
export default function Resrvations() {
  const { t } = useTranslation();
  const navigate = useNavigate();

  const [activeTab, setActiveTab] = useState(0);
  const token = localStorage.getItem("token")!;
  const { data: bookingPlacesRaw } = useGetAllBookingPlacesVendor(token);
  const { data: bookingServicesRaw } = useGetAllBookingServiceVendor(token);
